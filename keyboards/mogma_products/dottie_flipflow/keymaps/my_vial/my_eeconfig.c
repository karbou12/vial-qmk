...
void my_dump_eeconfig(const char* const func) {
    ...
    uprintf("------------------------------------------------------------\n");
    uprintf("%s DUMP EEPROM USER DATA. size:%u, defined size:%u\n", __FUNCTION__, sizeof(my_user_config), EECONFIG_USER_DATA_SIZE);
...
uint32_t my_get_offset(const my_user_config_field_e field) {
    switch (field) {
        ...
        case MY_FIELD_LAYER_TOGGLE ... MY_FIELD_SAME_VAL:
            return sizeof(my_hsvm_t) * ARRAY_SIZE(my_user_config.hsvm_layer) + sizeof(bool) * (field - MY_FIELD_LAYER7 - 1);
        default :
            return 0;
    }
}
    ...
    switch (keycode) {
        case MY_RESET:
            if (record->event.pressed) {
                rgblight_blink_layer_repeat(0, 300, 3);
